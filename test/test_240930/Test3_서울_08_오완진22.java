...
public class Test3_서울_08_오완진22 {
	...
	public static void main(String[] args) {
		...
		for (int tc = 1; tc <= TC; tc++) {
			...
			for (int n = 0; n < N; n++) {
				for (int m = 0; m < M; m++) {
					System.out.print(debugging[n][m] + " ");
				}
				System.out.println();
			}
			System.out.println();
...
//			minScore = Math.min(minScore, Math.abs(scoreB - scoreO));
		...
		// 추가한 디버깅 코드 ***************************************************************
		int score = Math.abs(scoreB - scoreO);
		if (minScore > score) {
			minScore = score;
			for (int n = 0; n < N; n++)
				for (int m = 0; m < M; m++)
					debugging[n][m] = visited[n][m];
		}
		// 추가한 디버깅 코드 ***************************************************************
	}
	
	// 추가한 메서드 ***********************************************************************
	// [0][0] 에서 시작하는 부분은 BTK으로 이어져 있기때문에 !visited 인 부분만 유효성 검사를 진행하면 될꺼라 생각함
	// = 오렌지 영역이 하나로 이어져있는지 끊어져 있는지 유효성 검사 메서드임
	static boolean isValid() {
		...
		for (int n = 0; n < N; n++) {
			for (int m = 0; m < M; m++) {
				...
				if (!visited[n][m] && !tmpVisited[n][m]) {
					bfs.add(new int[] {n, m});
					tmpVisited[n][m] = true;	// tmpVisited = 임시방문배열 (이 메서드에서만 유효함)
					cntO++;
					...
					while(!bfs.isEmpty()) {
						...
						for (int d = 0; d < 4; d++) {
							...
							if (nNext < 0 || N <= nNext || mNext < 0 || M <= mNext ||
									visited[nNext][mNext] || tmpVisited[nNext][mNext])	// 블루X & 방문X
								continue;
							
							bfs.add(new int[] {nNext, mNext});
							tmpVisited[nNext][mNext] = true;
						}
